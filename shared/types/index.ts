...
export const FileTransferGatewayEventSchema = z.discriminatedUnion('event', [
  z.object({ event: z.literal('transfer-initiated'), data: z.object({ uploadUrl: z.string(), fileKey: z.string(), stunServers: z.array(z.any()) }) }),
  z.object({ event: z.literal('transfer-status'), data: z.object({ fileKey: z.string(), status: z.string(), progress: z.number().optional() }) }),
  z.object({ event: z.literal('transfer-error'), data: z.object({ message: z.string() }) }),
