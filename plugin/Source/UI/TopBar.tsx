...
const TopBar: React.FC = () => {
  ...
  const [currentUserStatus, setCurrentUserStatus] = React.useState<Status>('online');
  ...
  const directMessageChannels = useMemo<Channel[]>(() => 
    (channels as Channel[]).filter((channel: Channel) => channel.type === 'direct'), 
    [channels]
  );
    ...
    <header className="bg-panel h-16 flex items-center justify-between px-6 rounded-t-xl shadow-outer-md text-text_primary">
              ...
              {isConversationOpen && (
                ...
                >
                  {directMessageChannels.length > 0 ? (
                    ...
                    <div className="px-4 py-2 text-sm text-text_secondary">No conversations</div>
                  )}
                </motion.div>
              )}
            </AnimatePresence>
      ...
      <div className="flex-1 max-w-2xl px-4 relative">
        <div className="relative">
          ...
          <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
            <svg className="h-5 w-5 text-text_secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex items-center space-x-6">
        ...
        <div className="relative">
          ...
          >
            <div className={`w-3 h-3 rounded-full ${statusColors[currentUserStatus]}`}></div>
            <span className="text-sm text-text_secondary">{statusLabels[currentUserStatus]}</span>
            <svg 
                  ...
                  {(['online', 'away', 'offline'] as Status[]).map((status) => (
                    ...
                    >
                      ...
                      {currentUserStatus === status && (
                        <svg className="w-4 h-4 ml-auto text-brand" fill="none" viewBox="0 0 24 24" stroke="currentColor">
