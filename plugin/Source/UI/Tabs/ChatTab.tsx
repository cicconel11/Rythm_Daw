...
const ChatTab: React.FC = () => {
  ...
  const channelParticipants = React.useMemo(() => {
    ...
    return users.filter(user => activeChannelData.participants.includes(user.id));
  ...
  if (!activeChannel) {
    return (
      <div className="flex items-center justify-center h-full bg-background">
        <p className="text-text_secondary">Select a channel to start chatting</p>
      </div>
    );
  }
        ...
        <div className="w-64 border-l border-gray-700 bg-panel flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            {channelParticipants.map(user => (
              <div key={user.id} className="flex items-center p-3 hover:bg-gray-700/30 transition-colors">
                <div className="relative">
                  <div className="w-9 h-9 rounded-full bg-gray-600 flex items-center justify-center text-white font-medium">
                    {user.name.charAt(0).toUpperCase()}
                  </div>
                  <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-panel ${
              ...
              <div className="flex items-center ml-2 space-x-2">
                ...
                {isDirectMessage && otherParticipants[0]?.status && (
                  <span className={`text-xs px-2 py-0.5 rounded-full ${
                    otherParticipants[0].status === 'online' ? 'bg-green-900/30 text-green-400' :
                    otherParticipants[0].status === 'away' ? 'bg-yellow-900/30 text-yellow-400' :
                    otherParticipants[0].status === 'busy' ? 'bg-red-900/30 text-red-400' :
                    'bg-gray-700 text-text_secondary'
                  }`}>
                    {otherParticipants[0].status === 'online' ? 'Online' :
                     otherParticipants[0].status === 'away' ? 'Away' :
                     otherParticipants[0].status === 'busy' ? 'Busy' : 'Offline'}
                  </span>
                )}
              </div>
            )}
