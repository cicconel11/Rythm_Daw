...
const Friends: React.FC = () => {
  ...
  const friends = {
    ...
    online: (currentUser?.friends || []).filter(
      (id: string) => users.find((u: User) => u.id === id)?.status === "online",
    ),
    ...
    <div className="flex flex-col h-full bg-background rounded-xl overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {activeTab === "add" ? (
          // Add Friends Tab
          <div>
            <h2 className="text-lg font-semibold text-text_primary mb-4">
              Add Friends
            </h2>
