...
describe("Auth E2E", () => {
  ...
  describe("Signup", () => {
    it("should create a new user", async () => {
      const response = await testRequest.post("/auth/signup").send(testUser);
  ...
  describe("Refresh", () => {
    it("should refresh tokens", async () => {
      ...
      const refreshResponse = await testRequest.post("/auth/refresh").send({
  ...
  describe("Logout", () => {
    it("should invalidate refresh token", async () => {
      ...
      const logoutResponse = await testRequest.post("/auth/logout").send({
        refreshToken: loginResponse.body.refreshToken,
      });
