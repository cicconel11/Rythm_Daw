...
interface AppState {
  ...
  addChannel: (channel: Omit<Channel, 'id' | 'unreadCount' | 'lastMessage'>) => void;
  ...
  persist(
    (set, get) => ({
      ...
      users: [
        ...
        { id: 'user4', name: 'Taylor Smith', status: 'offline', lastSeen: '2h ago' },
      ],
          ...
          channels: state.channels.map(channel => 
            channel.id === channelId
              ? { 
                  ...
                  unreadCount: state.activeChannel === channelId ? 0 : channel.unreadCount + 1
                }
              : channel
          )
        }));
      ...
      createGroup: ({ name, participants, description, isPrivate = false, avatar }) => {
        ...
        set((state) => ({
          channels: state.channels.map(channel => {
            if (channel.id === channelId) {
              return {
                ...channel,
                participants: [...new Set([...channel.participants, ...userIds])]
              };
            }
            return channel;
          })
        ...
        set((state) => {
          ...
          if (isCurrentUser) {
            return {
              ...
              activeChannel: state.activeChannel === channelId ? 'general' : state.activeChannel
            };
          }
