...
const stats = [
  { label: 'Active Plugins', value: '12', icon: Activity, color: 'text-[#7E4FFF]' },
...
const activities = [
  ...
  { action: 'Friend request from BeatMaker99', time: '2 hours ago', type: 'social' },
];
  ...
  return (
    <div className="min-h-screen bg-[#0D1126] bg-gradient-to-br from-[#0D1126] via-[#141B33] to-[#0D1126] p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-white font-['Inter'] mb-2">Dashboard</h1>
          <p className="text-gray-400">Welcome back to your music production hub</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat, index) => (
            <Card key={index} className="bg-gradient-to-br from-[#141B33] to-[#1A2142] border-gray-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2 bg-gradient-to-br from-[#141B33] to-[#1A2142] border-gray-700">
            <CardHeader>
              <CardTitle className="text-xl text-white font-['Inter']">Plugins</CardTitle>
            </CardHeader>
            <CardContent>
              <ScrollArea className="h-[300px]">
                <div className="space-y-3">
                  {plugins.map((plugin, index) => (
                    <div key={index} className="bg-[#0D1126] rounded-lg p-4 border border-gray-700">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          <div className={`w-3 h-3 rounded-full ${plugin.status === 'active' ? 'bg-green-400' : 'bg-gray-500'}`}></div>
                          <div>
                            <h3 className="font-semibold text-white">{plugin.name}</h3>
                            <p className="text-sm text-gray-400">{plugin.type}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="text-sm text-gray-400 mt-1">{plugin.cpu}</p>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <Card className="bg-gradient-to-br from-[#141B33] to-[#1A2142] border-gray-700">
            <CardHeader>
              <CardTitle className="text-xl text-white font-['Inter']">Recent Activity</CardTitle>
            </CardHeader>
            <CardContent>
              <ScrollArea className="h-[300px]">
